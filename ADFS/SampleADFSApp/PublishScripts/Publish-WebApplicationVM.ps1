...
      
	$msbuildCmd = '"{0}" "{1}" /T:Rebuild;Package /P:VisualStudioVersion=14.0;Configuration={3} /p:OutputPath="{2}\MSBuildOutputPath" /flp:logfile=msbuild.log' -f (Get-MSBuildCmd), $ProjectFile, $scriptDirectory,$BuildConfig
      
	Write-VerboseWithTime ('Build-WebDeployPackage: ' + $msbuildCmd)
      
	#Start execution of the build command
	$job = Start-Process cmd.exe -ArgumentList('/C "' + $msbuildCmd + '"') -WindowStyle Minimized -Wait -PassThru
    
	Write-VerboseWithTime "Job ID: $($job.id)"  
	...
	{
		Write-VerboseWithTime "Msbuil exit code: $($job.ExitCode)"
	}

	#Obtain the project name
	$projectName = (Get-Item $ProjectFile).BaseName
	Write-VerboseWithTime "projectName: $projectName"
      
	#Construct the path to web deploy zip package
	$DeployPackageDir =  '.\MSBuildOutputPath\_PublishedWebsites\{0}_Package\{0}.zip' -f $projectName
      
    Write-VerboseWithTime "DeployPackageDir: $DeployPackageDir"  
